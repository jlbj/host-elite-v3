...
import type { SavedTemplate, PropertyData } from '../models/paving.types';
...
function resolvePlaceholders(
  ...
  const getEquipmentIcon = (name: string): string => {
    ...
    if (n.includes('gym') || n.includes('fitness') || n.includes('sport')) return 'gym';
    if (n.includes('wine') || n.includes('bar') || n.includes('cigar')) return 'wine';
    if (n.includes('tv') || n.includes('télévision') || n.includes('television')) return 'tv';
    if (n.includes('audio') || n.includes('son') || n.includes('speaker')) return 'audio';
    if (n.includes('jacuzzi') || n.includes('bain') || n.includes('hot tub')) return 'jacuzzi';
    ...
    if (n.includes('beach') || n.includes('plage') || n.includes('sea')) return 'beach';
    if (n.includes('ac') || n.includes('clim')) return 'ac';
    return 'default';
  };

  const iconSvgMap: Record<string, string> = {
    pool: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="width:100%;height:100%"><path d="M2 20c2-2 4-2 6 0s4 2 6 0 4-2 6 0"/><path d="M2 16c2-2 4-2 6 0s4 2 6 0 4-2 6 0"/><path d="M12 2v10"/><path d="M8 6h8"/></svg>',
    spa: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="width:100%;height:100%"><path d="M4 12c0-2 2-4 4-4s4 2 4 4"/><path d="M12 12c0-2 2-4 4-4s4 2 4 4"/><path d="M4 20c0-2 2-4 4-4"/><path d="M16 16c2 0 4 2 4 4"/></svg>',
    cinema: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="width:100%;height:100%"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M9 8v8l6-4z"/></svg>',
    gym: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="width:100%;height:100%"><path d="M6 6l4 4"/><path d="M14 14l4 4"/><path d="M2 12h20"/><path d="M12 2v20"/></svg>',
    wine: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="width:100%;height:100%"><path d="M12 2v12"/><path d="M8 14h8v8H8z"/><path d="M6 10c0-3 2-5 6-5s6 2 6 5"/></svg>',
    default: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="width:100%;height:100%"><circle cx="12" cy="12" r="10"/><path d="M12 8v8"/><path d="M8 12h8"/></svg>',
  };

  const equipmentListHtml = equipments && equipments.length > 0
    ? equipments.map((eq: any) => {
        const symbol = getEquipmentIcon(eq.name || '');
        const svgContent = iconSvgMap[symbol] || iconSvgMap['default'];
        return `<div style="display:flex;align-items:center;gap:12px;font-size:0.95rem;color:#4c4c4c"><span style="display:flex;align-items:center;justify-content:center;width:24px;height:24px">${svgContent}</span><span>${eq.name || 'Amenity'}</span></div>`;
      }).join('')
    : '<div style="color:#94a3b8">No amenities listed</div>';

  const map: Record<string, string> = {
    title: data.listing_title || '[Title not loaded]',
    listing_title: data.listing_title || '[Title not loaded]',
    name: data.name || '[Name not loaded]',
    location: data.address || '[Location not loaded]',
    address: data.address || '[Address not loaded]',
    description: data.listing_description || '[Description not loaded]',
    listing_description: data.listing_description || '[Description not loaded]',
    guests: data.max_guests ? String(data.max_guests) : '[Guests not loaded]',
    max_guests: data.max_guests ? String(data.max_guests) : '[Guests not loaded]',
    bedrooms: data.bedrooms ? String(data.bedrooms) : '[Bedrooms not loaded]',
    bathrooms: data.bathrooms ? String(data.bathrooms) : '[Bathrooms not loaded]',
    rooms: data.rooms ? String(data.rooms) : '[Rooms not loaded]',
    sqm: data.surface_area ? String(data.surface_area) : '[SQM not loaded]',
    surface_area: data.surface_area ? String(data.surface_area) : '[SQM not loaded]',
    bed_count: data.bed_count ? String(data.bed_count) : '[Bed count not loaded]',
    property_type: data.property_type || '[Property type not loaded]',
    rental_mode: data.rental_mode ? String(data.rental_mode) : '[Rental mode not loaded]',
    house_rules: data.house_rules_text || '[Rules not loaded]',
    house_rules_text: data.house_rules_text || '[Rules not loaded]',
    wifi_code: data.wifi_code || '[WiFi code not set]',
    arrival_instructions: data.arrival_instructions || '[Arrival instructions not set]',
    emergency_contact: data.emergency_contact_info || '[Emergency contact not set]',
    cleaning_contact: data.cleaning_contact_info || '[Cleaning contact not set]',
    ical_url: data.ical_url || '[iCal URL not set]',
    cover_image: data.cover_image_url || (photos && photos[0]?.url) || '',
    cover_image_url: data.cover_image_url || (photos && photos[0]?.url) || '',
    price: '0',
    price_formatted: '0',
    currency: '€',
    period: 'week',
    price_period: 'week',
    has_pool: hasPool ? 'yes' : 'no',
    pool_status: poolStatus,
    has_spa: hasSpa ? 'yes' : 'no',
    spa_status: spaStatus,
    amenities_count: equipments ? String(equipments.length) : '[Not loaded]',
    photos_count: photos ? String(photos.length) : '[Not loaded]',
    equipment_list: equipmentListHtml,
    photo_1: photos && photos[0]?.url || (data.cover_image_url || ''),
    photo_2: photos && photos[1]?.url || (data.cover_image_url || ''),
    photo_3: photos && photos[2]?.url || (data.cover_image_url || ''),
  ...
  async ngAfterViewInit() {
    ...
    try {
      ...
      this.editor.on('load', () => {
        ...
        B('he-hero', { label: 'Hero', category: 'Property', media: s+'<rect x="2" y="3" width="20" height="18" rx="2"/><path d="M2 12h20"/></svg>', content: '<div class="luxury-hero" data-hero-title="{{title}}" data-hero-subtitle="{{property_type}}" data-hero-bg="{{cover_image}}" style="position:relative;height:70vh;background-image:url({{cover_image}});background-size:cover;background-position:center;display:flex;align-items:flex-end;padding:60px 4%"><div style="position:absolute;inset:0;background:linear-gradient(to bottom,rgba(0,0,0,0.1),rgba(0,0,0,0.6))"></div><div style="position:relative;z-index:10;color:#fff"><span style="font-size:1.1rem;text-transform:uppercase;letter-spacing:0.15em;opacity:0.9">{{property_type}}</span><h1 style="font-size:3.5rem;font-weight:400;margin:0 0 10px;letter-spacing:-0.02em;font-family:Playfair Display,serif">{{title}}</h1><span style="font-size:1.1rem;text-transform:uppercase;letter-spacing:0.15em;opacity:0.9">{{location}}</span></div></div>' });
        B('he-stats', { label: 'Stats Bar', category: 'Property', media: s+'<path d="M2 4v16"/><path d="M2 8h18a2 2 0 0 1 2 2v10"/><path d="M2 17h20"/></svg>', content: '<div class="luxury-bar" data-guests="{{guests}}" data-bedrooms="{{bedrooms}}" data-bathrooms="{{bathrooms}}" style="border-bottom:1px solid #eaeaea;padding:24px 4%;display:flex;gap:40px;font-size:0.9rem;text-transform:uppercase;letter-spacing:0.08em;color:#757575;background:#fafafa"><span>Guests <strong>{{guests}}</strong></span><span>Bedrooms <strong>{{bedrooms}}</strong></span><span>Bathrooms <strong>{{bathrooms}}</strong></span><span>Pool <strong>{{pool_status}}</strong></span></div>' });
