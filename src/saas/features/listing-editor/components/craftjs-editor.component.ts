...
function resolvePlaceholders(html: string, data: PropertyData): string {
    ...
    try {
      ...
      this.editor.on('load', () => {
        ...
        this.editor.on('component:create', (component: any) => {
          ...
          const resolved = resolvePlaceholders(el_.innerHTML, this.store.propertyData());
          ...
          for (const attr of el_.attributes) {
            if (attr.value.includes('{{')) {
              attr.value = resolvePlaceholders(attr.value, this.store.propertyData());
            }
          }
        });
