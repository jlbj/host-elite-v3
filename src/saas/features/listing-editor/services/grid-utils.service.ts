...
import type { GridBlock } from '../models/paving.types';
...
export class GridUtilsService {
  computeSeparators(blocks: GridBlock[], _containerWidth: number): Separator[] {
    const separators: Separator[] = [];

    const horizontalPositions = [...new Set([
      ...blocks.map(b => b.bounds.top),
      ...blocks.map(b => b.bounds.bottom),
    ])].sort((a, b) => a - b);
    let hIndex = 0;
  ...
  refreshLayoutPositions(blocks: GridBlock[]): GridBlock[] {
  ...
  applyDragToBlocks(
    ...
    const result = kept.length !== next.length ? this.refreshLayoutPositions(kept) : next;
  ...
  mergeBlocks(
    ...
    return this.refreshLayoutPositions([...blocks.filter(b => b.id !== blockId1 && b.id !== blockId2), merged]);
  }
}
